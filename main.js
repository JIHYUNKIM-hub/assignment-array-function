...
const length = names.some(function (name) {
  let temp2 = name.trim();
  return temp2.length >= 20;
});
console.log("some문제1:", length);
// 성을 제외한 이름에 p를 포함한 사람이 있는가?(대소문자 상관 no)
const includeP = names.some(function (name) {
  return name.split(" ")[0].toLowerCase().includes("p");
});
console.log("some문제2:", includeP);

// (4)every 문제
// 모두의 전체 이름의 길이가 20자 이상인가?
const length2 = names.every(function (name) {
  let temp4 = name.trim();
  return temp4.length >= 20;
});
console.log("every문제1:", length2);
// 모두의 이름에 a 가 포함되어 있는가?
const includeA = names.every(function (name) {
